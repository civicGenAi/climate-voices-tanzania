      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        >
          <div className="flex-shrink-0 w-24 h-24 sm:w-32 sm:h-32 md:w-40 md:h-40 rounded-xl sm:rounded-2xl bg-parchment border border-forest/10 flex items-center justify-center p-3 sm:p-4">
            <img src={chanyaLogo} alt={t("partners.chanyaName")} className="w-full h-full object-contain" />
          </div>
          ...
          <div className="flex-1 text-center sm:text-left">
            <div className="flex items-center justify-center sm:justify-start gap-2 mb-2 sm:mb-3">
              ...
              <span className="font-mono text-[10px] sm:text-xs text-gold tracking-wider uppercase">{t("partners.officialPartner")}</span>
            </div>
            <h3 className="font-display text-xl sm:text-2xl md:text-3xl font-bold text-parchment-foreground mb-2 sm:mb-3">
              {t("partners.chanyaName")}
            </h3>
            <p className="font-body text-xs sm:text-sm text-parchment-foreground/70 leading-relaxed mb-4 sm:mb-6">
              {t("partners.chanyaDescription")}
            </p>
            <a href="#" className="inline-flex items-center gap-2 text-forest font-body font-medium text-xs sm:text-sm hover:text-gold transition-colors">
              {t("partners.visitWebsite")} <ExternalLink className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
            </a>
          </div>
        </motion.div>
